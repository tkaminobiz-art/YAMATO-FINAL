...
const url=process.env.WORKS_URL||'http://127.0.0.1:4175/works.html',phase=process.env.WORKS_PHASE||'local',dir='qa/works-recovery-20260908';
const report={url,phase,at:new Date().toISOString(),views:[],tests:[],errors:[]};
const counts={home:6,saki:10,shijo:4,sakyo:20,miyamaki:13};
...
for(const [engine,type,widths]of [['chromium',chromium,[1440,390,320]],['webkit',webkit,[1440,390]]]){
 ...
 for(const width of widths){
  ...
  const d=await p.locator('#worksPhotoData').textContent().then(JSON.parse);assert.equal(d.archive.length,20);assert(!d.galleries.home.images.some(i=>i.src.includes('jittei_living_03')||i.src.includes('jittei_ext_02')));
  ...
  for(const [id,n]of Object.entries(counts)){
   ...
   await p.keyboard.press('ArrowRight');await loaded(p);
   for(let k=0;k<n;k++){
    assert.equal(await p.locator('.viewer-image').getAttribute('src'),d.galleries[id].images[k].src);
    await p.locator('.viewer-image').evaluate(i=>i.decode());
    if(width!==320 && (id==='home'&&k===4 || id==='sakyo'&&k===16 || id==='miyamaki'&&k===5))await p.screenshot({path:`${dir}/${phase}-${engine}-${width}-${id}-changed.png`,scale:'css'});
    await p.keyboard.press('ArrowRight');await loaded(p);
   }
