...
export function renderSearch(store,{rebuild=true}={}){
 ...
 document.querySelector('#match-note').textContent=schoolTypes[s.sort]?`${schoolTypes[s.sort]}までの掲載距離が短い順です。距離の情報がない物件は最後に表示します。`:s.sort==='updated'?'情報提供日が新しい順です。':rows.some(r=>r.matchStatus==='plot-unverified')?'価格や面積に幅がある物件を含みます。ご希望に合う区画は担当者にご確認ください。':'価格・面積・徒歩時間はSUUMOの掲載情報です。';
