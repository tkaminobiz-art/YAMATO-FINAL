...
export const choices={
 family:[['unknown','まだ確認していない'],['none','該当しない'],['young','若年夫婦世帯に該当'],...[...Array(22)].map((_,i)=>[String(i+1),`対象となるこども ${i+1}人`])],
 performance:[['unknown','まだ確認していない'],['none','引下げなし'],['zeh','S（ZEH）に適合する場合'],['a','S（金利Aプラン）に適合する場合'],['b','S（金利Bプラン）に適合する場合']],
 maintenance:[['unknown','まだ確認していない'],['none','利用しない'],['long','長期優良住宅の認定を受ける場合']],
 region:[['unknown','まだ確認していない'],['none','利用しない'],['child','地域連携型（子育て支援・空き家対策）'],['active','地域連携型（地域活性化）'],['migration','地方移住支援型']],
 site:[['unknown','まだ確認していない'],['eligible','引下げ制度の立地要件を満たす場合'],['excluded','引下げ制度の対象外区域に該当']]
};
...
export function flatReduction(raw={}){
 ...
 if(f.maintenance==='long'&&!['zeh','a'].includes(f.performance))throw Error('この長期優良住宅の引下げは、住宅性能のZEHまたは金利Aプランとの併用条件を確認してください。');
 ...
 return {points,reductions,unknowns,assumption:reductions.length?'選択条件で金利引下げを利用できた場合':'選択条件による参考試算・引下げ加算なし'};
}
...
export function financeAssumption(f,productId){f=normalizeFinance(f);return productId==='nanto'?(f.nantoIncrease?`借入当初から金利が${f.nantoIncrease}ポイント高かった場合`:'最大引下げ金利が返済中変わらない場合'):flatReduction(f).assumption;}
